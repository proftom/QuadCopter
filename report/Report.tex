...
\begin{itemize}
	\item What are we trying to achive
	\item Why is this a good idea to achive
	\item how are we going to achive it
\end{itemize}
...
We chose to use an approach that does not rely on an aerodynamic model of the craft, similar to the approach in \cite{OpenPilotPaper}. This has the advantage that modelling errors are minimised, and the time required for system identification is reduced.
...
as such we cannot anticipate 

de facto standard (cite)\cite{todo}



The \ac{EKF} is the non-linear version of the regular Kalman Filter, (linearises the system around the current estimate)



The filter is based on the following descrete time update model
