...
The PCA was never implemented as it was found that by increasing the minimum number of points in the fill region for that region to be considered a plane in the     lkjlj. Additionally, it was decided to remove the use of DBScan altogether as it was realised that although planes occasionally become segmented using Flood Fill, in practice it made no measurable difference to the result of the Kalman filter.

after thinking ahead towards plane association in the state update of the EKF (below), it was realised that although planes occasionally become segmented using Flood Fill, in practice it made no measurable difference to the result of the Kalman filter. Additionally, by increasing the minimum number of points ...  
 
% recieved I would be simpler to pass every plane found from Flood Fill. the Kalman filter itself can and should handle all planes from the  DBScan algorithm or PCA was required. In fact, it was found that DBScan was no longer the algorithm suitable, but rather a similar algorithm called Iterative Nearest Neighbour (INN) should be used. This is discussed in detail in the registration section of the EKF. Fruther PCA was neither implemented as Initial attempts to resolve this problem by adjusting the flood fill algorithm parameters such as the minimum points to consider a plane failed.
...
The project was created with the goal to use inexpensive and available hardware to accurately estimate the state of a . 
