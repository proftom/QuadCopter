...
%todo
localisation is done through a ekf

which is a filter for blablabla, maybe diagram

<<talk about just imu being hrap?

the primary sensor for feature extraction is a depth camera, streaming qqvga at 30 fps. ONLY DEPTH CHANNEL IS OF INTEREST TO US

openni + pcl gives us data in point cloud form (xyz) relative to the camera, and the ability to operate on the cloud

from these point clouds, integral image estimation to get local (Nx, Ny, Nz) normal vector at each point in the cloud. Then a Flood Fill algorithm is used to cluster points with similar local normals. Note that the region growing for the clusters is done in the xyz space but the condition for adding a point to the cluster operates in nxnynz space.

Flood Fill, for a point cloud data obtained from one depth frame. starting from a set of randomised seed points... for each seed point, the algorithm visits its neighbours in the camera-xy space 

patches thrown into the kalman filter, and associated based on mahalanobis distance

include the fact that dbscan was toyed with at the start?
...
This has the advantage that modeling errors are minimised, and the time required for system identification is reduced.
Complex phenomena such as the turbulence caused by the thrust channeled around in the confined space would be close to impossible to model.
...
For compatibility with the Kalman filter, we assumed the noise in the system to be Gaussian white noise as follows
\begin{align}
	w_k &\sim N(0,Q_k) \notag \\
	v_k &\sim N(0,R_k) \notag \\
...
To test the system, a yaw control loop was built where the system uses the estimated state to control the quad copter's yaw such that the depth camera (and hence frame of the craft) is always pointing towards a set point in the global coordinate system. The system was tested under lab conditions, where 2 intersecting planes were constructed to be used as a features for the system to localise against. The system was initialised to a known location, with the planes well within the depth camera's field of view. The quad copter was controlled remotely, and flown into a stable position out of ground effect before manually switching control of the yaw component to the automated control loop. So that the system never loses visibility of the planes, the chosen point to track was the corner between the 2 constructed planes and the floor.  
