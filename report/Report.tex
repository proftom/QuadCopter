...
To properly running the kalman filter, we had to tune the measurement uncertainty parameter since the order of covariance obtained by using the RANSAC algorithm was too low. We multiplied the uncertainity of the Xtion by  a factor of 100000 which seems too big but through testing, we found this to be the ideal factor. A larger factor would result in the kalman relying too much on the inertial sensors  while a smaller one means we trust the xtion's output more. Since both sensors are noisy, an optimal value was selected.
We then perform some analysis on our system to check that our filter works correctly. Firstly we checked for consistency in the kalman filter by verifying that errors in the observation measurements are bounded to $\pm\sqrt{2\sigma}$. We found this to be the case that at least 95\% of our errors are within this bound. The second test was to verify that there was no bias in our observation errors as the filter evolves. To do this, we performed the autocorrelation of the noise and found it to be bias free. [Graph given below].
