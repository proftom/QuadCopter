...
\author{Oskar Weigl (ow610), Ryan Savitski (rs5010), Thomas Morrison (tm1810), \\
		Chinemelu Ezeh (cee110), Joshua Elsdon (je310)}
\begin{document}
\maketitle
...
\begin{figure}[p]
	...
	\caption{Effects of the low pass filtering by the normal estimation kernel. White lines represent local surface normals.}
...
The downsides to the DBScan approach is the computational and space complexities. With $n$ being the number of points per frame, there is an O(n) preprocessing step to find $D$ components, followed by $O(n^2)$ clustering without accelerating data structures, or $O(n \cdot log(n))$ with. Therefore the decision was made to instead implement a faster flood fill algorithm and leave the coplanar patch merging to the EKF's feature association stage.
...
% Modifications to the DBScan algorithm were made to decrease the running time. By keeping an open and a closed node sets (a point can belong to one cluster only), thus removing redundant duplicate lookups from the region query. Further a custom conditional insertion procedure to insert into a list-like data structure without duplication was created, decreasing the complexity of insertion from $O(m \cdot log(n))$ (if using the standard template library set object) to $O(m)$.
...
% we considered doing the below but realised that kalman can do stitching in plane association 
...
The flood fill algorithm is analogous to a paint bucket function in a graphical editing package. The algorithm begins by randomly selecting a seed plane from the plane cloud (that is we randomly select a point and corresponding normal from the RGB-D camera sensor output), and proceed to grow a cluster out from this seed in all directions until the local surface plane equation parameters at those points $(N_A, N_B, N_C, D)$ differ by more than a value $\epsilon$ from the original seed. This forms a connected region, which represents a planar patch. Algorithm~\ref{alg:flood_fill} lists how, given a seed point, a region fill is peformed.

The fill is then performed again, selecting a new random seed point from a point that has not been already included in a previous fill region. The algorithm will repeat until a certain portion of the image is filled (called $\sigma$) or there have been more than $\gamma$ seed points tried on the frame. The flood fill algorithm works iteratively with a linear complexity of $O(n)$. An example of the clustering as done by the flood fill is shown in Figure~\ref{fig:flood_fill}.
...
\begin{figure}[p]
	\centering     %%% not \center
	\subfloat[No clustering]{
		\label{fig:6box_before}
		\includegraphics[width=0.4\textwidth]{6box_before.PNG}
	}
	\;
	\subfloat[Plane clustering]{
		\label{fig:6box_after}
		\includegraphics[width=0.4\textwidth]{6box_afterward.PNG}
	}

	\caption{Plane finding using DBScan}
...
\begin{figure}[p]
	...
	% \subfloat[Result of Flood Fill, $\epsilon=0.2, \gamma=50, \sigma=0.8$] 
	% {
	% 	\label{fig:dbscan_planes}
	% 	\includegraphics[width=0.4\textwidth]{floodFill.png}
	% }
	% \;
	...
	% }
	\caption{Plane clustering using flood fill, $\epsilon=0.2, \gamma=50, \sigma=0.8$}
...
To estimate the state of the system we employ the EKF which is the non-linear version of the regular Kalman Filter: the optimal state estimation filter for linear systems \cite{Stengel} with Gaussian noise.
When supplied with an accurate system model, and its statistical properties, the Kalman Filter provides the state estimate with the minimum mean-squared error \cite{oxford}.
...
In practice, the assumption of approximately Gaussian noise works well in practice, and is used in many navigation applications \cite{Stengel}.
...
When a new plane measurement is generated by the depth camera and the vision system, we need to decide whether this plane exists in the map or not. To do this we use the statistical measure called the squared Mahalanobis distance to evaluate how closely a newly observed plane is related to the ones in the state map.
This measure is best described as a normalised distance that is used to gauge similarity of a new sample to a distribution with a known covariance matrix. The measure is formally defined as
\begin{align}
	d &= (x-\mu)^\top S^{-1} (x-\mu)
\end{align}
where $\mu$ is the mean of the distribution and $S$ is its covariance matrix. In our case, $\mu$ is the estimate of a plane equation which is in our map, and $S$ is defined in Equation~(\ref{eqn:measurement_model}).
...
\verb"d = chi2inv(0.99,4) = 13.2767" which is the MATLAB command for the inverse chi squared distribution with 4 degrees of freedom, evaluated at 0.99.

For each plane received from the vision algorithm, we compute the Mahalanobis distance to each plane in the map.
If there are any pairs that have a distance less than the threshold, we associate it to the landmark with the smallest one.
If no matches are found, we add the plane to the plane map as described in the next section.

% subsection landmark_association (end)

\subsection{Landmark Initialization} % (fold)
\label{sub:landmark_initialization}
The ability to observe new planes as the craft explores its environment is crucial for it to keep track of its location. By re-observing these new features, the craft is able to compute corrections in its location.
When the craft sees a new plane, it adds this to its internal state by transforming the plane equations in the body frame to the world frame and then storing this and its associated covariance information. This transformation we call $e$, the inverse observation model, which describes the state of the plane in the form we store in our map from an observed plane equation $y$. Mathematically we have
\begin{align}
	y_{k} &=  
	\left[
	\begin{matrix}
	...
	\right]
\end{align}
where $T_e$ defines the transformation matrix from the body frame to the earth frame. The new plane, numbered $j$ is defined as 
\begin{align}
	\pi_j	&= e(y_k,x) \notag \\
			&= T_e y_k
\end{align}

We now add the plane to our state as follows
\begin{align}
	x_{k} &\leftarrow
	\left[
	\begin{matrix}
...
To initialize the new entries in the state covariance matrix, $P$, we have to compute the Jacobian of $e$ w.r.t the state, which we denote as $E_x$, and the Jacobiain of $e$ w.r.t the newly observed plane, denoted as $E_y$. Mathematically we have
\begin{align}
...
Using the optimizations described in \cite{Sola2013} we use these matrices to initialize the new entries in $P$ as follows
\begin{align}
...
It is important to note that we do not do any de-noising checks on the new planes that are being registered in our plane map. 
...
A widely used technique, called serial updating, can be used to speed up the computation of the Kalman filter update equations when there are many uncorrelated measurements. This is described in \cite{OpenPilotPaper} and chapter 4.2.2 of \cite{KFBookSerialupdate}, and also mentioned in \cite{Sola2013}.
...
We, on the other hand, use a measurement model that includes the measurement of planes. These measurements consist of the estimated plane equation in Hessian Normal form. As the plane actually lies in three dimensional space, and thus the measurement error will have three degrees of freedom, and the aforementioned representation has four dimensions, the covariance matrix cannot be uncorrelated.
More specifically, as the Hessian Normal form uses a unit vector, the length of this vector is bound to unity. As such, the probability distribution of the error of the components of this normal vector cannot be spherical.
In fact, after linearisation, this distribution is projected onto a hyper-plane tangential to the actual distribution.

While we cannot serialise the individual components of a plane measurement, the measurement errors between simultaneous plane measurements are uncorrelated, so we can still make use of a hybrid serial update strategy. That is, at each time there is a new frame that has finished feature extraction, we serially apply the measurement updates for each plane, without running the predict loop in-between.
Assuming that there are $v$ measured plane equations at time $k$, we have that our algorithm will only have to invert $v$ matrices of size $4\times4$ at the update stage, instead of inverting a $4v \times 4v$ matrix.

As mentioned briefly in in Section~\ref{sub:system_model}, the most important optimization is to that the equation that transforms the state covariance matrix (\ref{eqn:predictP}) forms a separable system. That is, the Jacobian matrix $F$ as defined in (\ref{eqn:F}) does not let the state of the craft affect the the plane map, and vice versa.
...
This lends itself to the optimisations excellently described in \cite{Sola2013}.
This makes an enormous difference in the scalability of the Kalman filter approach. If the entire matrix-matrix multiplication is required in equation \ref{eqn:predictP}, then the algorithm will scale like $O(n^3)$ where $n$ is the number of planes in the state vector. Now that this cubic dependence on $n$ is removed, only $O(n^2)$ terms remain from the rest of the filter, along with lower order terms, making the filter perform each update stage in $O(n^2)$ time.
...
In our test setup, we based our coordinate system in a vertex of the room as origin defining the x,y and z coordinates to be along the three wall edges connected to the origin vertex. Our test system consist of Mavrx's inertial measurement unit containing the inertial sensors and depth camera.
To ensure a functional kalman filter, full observability of system in the x,y and z axis is needed. Ideally, an observation should be a plane equation in space giving information in all the components of the Nx, Ny, Nz and D spaces. Planes however, fundamentally provides accurate information in at most two of the three normal dimensions, and distance.
Thus we decided to observe three orthogonal planes formed by the wall that meet at the origin in order to achieve full observability. We then initialise our state in the filter to be -2,2,2 in the x,y,z space. In practice however, the initial state will be close to but not exactly the same as in the filter. It is left to the kalman filter to converge to the real state estimate.

To ensure correct operation of the kalman filter, we had to tune the observation covariance parameter since the range of values obtained by using our custom feature extraction algorithm was too low (in the order of $10^{-7}$). A larger value would result in the kalman relying too much on the inertial sensors while a smaller one means we trust the depth camera's output more [Fig:~\ref{fig:trustXtion}].
Feature extraction has considerable random noise due to the random initial seed point for the custom flood fill algorithm. Thus trusting the camera would result in high frequency noise or fluctuations in state estimate. On the other hand, position estimate from the inertial sensors behave like low pass filtered values as it is obtained by double integrating the acceleration. Thus trusting the inertial sensors more by increasing the depth cameras uncertainty results in a lower frequency in estimation.
We choose an appropriate value such that our filter behaves within an optimal region based on three test as discussed in \cite{KalmanTuning}. We focus our analysis on position estimate alone because since it depends on speed and attitude change, a good position estimate is sufficient to validate our algorithm implying the other estimates are correct. 
...
\section{Conclusion}
An off-the-shelf depth camera designed to track body movements (and hence had no need for precise distance measurements) was calibrated using a custom calibration method. After this the point clouds generated gave accurate values and a distortion free image. This was instrumental for the detection of planes to work reliably. 

Selection of an appropriate computing platform was preformed, and showed that support for the ARM application processors and the software tools that we were using was not mature, and implementation was not tenable in the time frame. The final implementation used an x86 mini-ITX board, which worked successfully but was suboptimal for weight and for connectivity to custom peripherals (specifically SPI).
...
An extension of this system was also designed such that new planes could be added to the internal plane map. The implementation was not completed, though all of the theoretical work has been presented in this report.

When mounted on an airframe the implementation successfully controlled the yaw of the craft successfully, though the response was slow due to over correction. It has been hypothesised that the reason for the over correction in yaw at higher speeds is due to latency in the communication between the x86 platform and the R10 quad copter flight controller hardware. The slow communication was due to serial links through the SPI bridge as the x86 platform had no SPI port.

The final implementation is available at \url{https://github.com/proftom/QuadCopter}.
