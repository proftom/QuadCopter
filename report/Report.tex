...
The intended approach is to use a consumer grade RGB-D camera as the primary sensor, supplemented with accelerometers, gyroscopes and a downward facing sonar sensor. In order to generate the estimate of the robot's state we use an Extended Kalman Filter (EKF) to fuse the data. To simplify feature extraction and reduce the computational complexity, features tracked by the filter are planar surfaces. This is due to planes are being easy to characterise from all directions as well as being a consistently available feature in man-made environments. 

The depth camera of choise is the Asus Xtion Pro as it is considerably lighter than the copeting products, while maintaining a similar price point. Due to the relatively narrow field of view (58$^{\circ}$ horizontal, 45$^{\circ}$ vertical), the floor and the ceiling is expected to often be out of view, resulting in reduced observability in the elevation direction. The depth camera is complemented with a downward facing sonar sensor to address this issue.
...
To obtain consistent and robust computer vision the open source PCL was used as a foundation on which our vision algorithms were implemented. By leveraging the both well featured and documented PCL allowed us to rapidly develop and test our algorithms in real time. Among other features, the PCL library provided an interface to the Xtion hardware (RGB-D camera), exposing the data captures known as point clouds at speeds of up to 30 frames per second.
...
When running the system on bench (RGB-D camera with the IMU sensor board) performs well, with a strong resilience to noise.


It has been hypothesized that the reason for the over correction in yaw at higher speeds is due to the hardware communication delay between the embedded platform and the R10 quad copter flight controller. The flight controller only supported control over a SPI bus, while the x86 board which ran the vision and Kalman filter had no such interface. To enabled communication between the hardware, a small embedded processor known as an MBed was used. This processor ran a software SPI to USB bridge, allowing the x86 to connect to the flight controller through a USB port. This bridge combined with a USB . 
The total delay or latency between the x86 and 

Solutions to this problem are 


It is believed that the delay modifications to the firmware were required 
is not an inherent property of the algorithms used, but rather the 
